...
// 지원 체계 연동 (Tier 1 학급 보편 · Tier 2 소그룹/CICO) — 한 칸 표.
function tierLinkageBlock(tierLinkage) {
  const body = tierLinkage && String(tierLinkage).trim()
    ? escML(tierLinkage)
    : ph('학급 PBS·소그룹(CICO) 운영 내용이 없습니다. (Tier 1/Tier 2 화면에서 입력하면 자동 연동됩니다)');
  return `<table class="t3"><tr><td>${body}</td></tr></table>`;
}
 ...
 * @param {string} opts.tierLinkage      - Tier1(학급 보편)·Tier2(소그룹/CICO) 연동 텍스트(없으면 안내문)
 */
export function downloadTier3Doc({
  ...
  const html =
    ...
    `<h2>Ⅲ. 지원 체계 연동 (Tier 1 학급 보편 · Tier 2 소그룹)</h2>${tierLinkageBlock(tierLinkage)}` +
    `<h2>Ⅳ. 모니터링 및 평가 계획</h2>${monitorBlock(monitorRows)}` +
