...
// 학교 규칙 수립을 위한 조사서 (Tier 1) — 반·학기 단위 1행.
//   GET  /api/school-rules-survey?class_id=ID&semester=1
//   POST /api/school-rules-survey  body { class_id, semester, responses }
// 자가치유: 테이블이 없으면 생성(마이그레이션 전에도 동작) — class-pbs-survey와 같은 패턴.
export default requireAuth(async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const r = await sql`
        SELECT * FROM school_rules_survey WHERE class_id = ${classId} AND semester = ${semester}
      `;
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      const r = await sql`
        INSERT INTO school_rules_survey (user_id, class_id, semester, responses, updated_at)
    ...
    console.error('School rules survey error:', e);
