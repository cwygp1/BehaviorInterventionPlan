...
const PRESETS_20 = [
  ...
  { id: 5, icon: '💬', tag: '웹 앱', title: 'AAC 보드 — 일상생활 의사소통', presets: { 장애: ['🤝 중복장애'], 수준: ['🟢 입문'], 결과물: ['💬 AAC 보드 HTML'], EBP: ['🃏 PECS'], 접근성: ['🔊 TTS(자동 낭독)', '👆 큰 터치', '📱 AAC 연계'] }, topic: '카테고리: 인사·요구·거부·감정\n각 카테고리당 6개 픽토그램\n클릭 시 TTS 자동 발화' },
  ...
  { id: 8, icon: '📅', tag: '웹 앱', title: '하루 시각 일과표', presets: { 장애: ['🔵 자폐성장애'], 결과물: ['📅 시각 일과표'], 접근성: ['🖼 픽토그램', '⏸ 자기 속도'] }, topic: '하루 7시간 일과(등교~하교)\n각 활동: 시간·이름·픽토그램\n완료 시 체크 + 다음 활동 강조' },
...
function findDisabilityChips(disability) {
  ...
  const chips = splitDisability(disability)
    .map((d) => items.find((it) => {
      const name = it.replace(/^\S+\s/, ''); // 이모지 제거
