...
// 2026-08: 표적행동 우선순위·강화제 체크리스트를 모든 AI 생성기가 같은 형태로 받도록 공용화.
import { priorityLines } from './priority';
import { rcLines } from './reinforcerChecklist';
...
export function raisdLines(data) {
  ...
  // 강화제 체크리스트(선호도 평정) 상위 항목 — 강화 전략에 그대로 쓸 수 있는 구체 항목.
  rcLines(meta.checklist).forEach((l) => lines.push(l));
...
export function buildStudentSummary({ student, data, startpoint } = {}) {
  ...
  if (abc.length) {
    ...
    abc.slice(-3).forEach((r) =>
      lines.push(`  · ${(r.setting_event || r.setting) ? `[배경사건 ${r.setting_event || r.setting}] ` : ''}선행 "${r.antecedent || r.a || ''}" → 행동 "${r.behavior || r.b || ''}" → 후속 "${r.consequence || r.c || ''}"`)
    );
  }
  const mon = data?.mon || [];
  ...
  // 표적행동 우선순위 체크리스트 — 교사가 "무엇부터 중재할지" 정해둔 결과. 중재 목표 선정의 근거.
  priorityLines(data?.priority?.responses).forEach((l) => lines.push(l));
