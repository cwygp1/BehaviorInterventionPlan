...
function goalBlockText(g, isDaily) {
  ...
  parts.push('＊평가방법: ' + critMethodText(g.crit_type, g.crit_of));
  if (foci.length) {
    parts.push('＊평가초점');
    ...
    const form = fullTable([
      new TableRow({
        children: [
          ...
          headCell('학년', COLS[2]), cell(g.subject ? (GRADE[g.grade_code] || '') : '', { width: COLS[3], center: true }),
        ],
      }),
      new TableRow({
        children: [
...
export async function downloadTaskSheetDocx({ student = {}, teacherName = '', goalText = '', steps = [], chainType = 'forward', promptSystem = 'mtl', totalCols = 8 }) {
  ...
  const rows = [
    new TableRow({
      children: [
        ...
        headCell(`회기 / 날짜 (각 칸에 + − P I 기록)`, 60, { colSpan: totalCols }),
      ],
    }),
    new TableRow({
      children: [
    ...
    new TableRow({
      children: [
        cell(`정반응(+·I) 단계 수 / 전체 ${clean.length}`, { width: 40, colSpan: 2, center: true, bold: true, shade: true }),
        ...blankCells(),
      ],
    }),
  ];
  ...
  const legend = [
    new Paragraph({
      ...
      children: [new TextRun({ text: '기록 코드   + 정반응 | − 오반응 | P 촉구 | I 독립 | T 교사 수행(아직 가르치지 않는 단계)', font: FONT, size: 18, bold: true })],
    }),
    new Paragraph({
      children: [new TextRun({ text: `회기마다 각 단계에 위 코드를 적고, 맨 아래 행에 그날 정반응(+ 또는 I) 단계 수를 적습니다. 촉구 종류를 나눠 적고 싶으면 P 옆에 언어·몸짓·시범·신체를 덧붙이세요. (${chain} · ${prompt})`, font: FONT, size: 18 })],
    }),
  ];
