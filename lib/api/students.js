...
// 샘플 체험 — 현재 학급에 샘플 학생 2명 + 4주치 기록 시드 / 일괄 삭제
export const seedSampleStudents = (class_id) => api('/api/students/sample', 'POST', { class_id });
export const clearSampleStudents = () => api('/api/students/sample', 'DELETE');
...
export const updateMonitor = (sid, body) => api(`/api/students/${sid}/monitor`, 'PUT', body); // 0719: 기록 수정
...
// Tier 1 class PBS — scoped to (class_id, semester).
export const fetchClassPBS = (classId, semester = 1) =>
  api(`/api/class-pbs?class_id=${encodeURIComponent(classId)}&semester=${encodeURIComponent(semester)}`);
export const saveClassPBS = (body) => api('/api/class-pbs', 'POST', body);

// PBS 기초 설문조사 (Tier 1) — 반·학기 단위
export const fetchPbsSurvey = (classId, semester = 1) =>
  api(`/api/class-pbs-survey?class_id=${encodeURIComponent(classId)}&semester=${encodeURIComponent(semester)}`);
export const savePbsSurvey = (body) => api('/api/class-pbs-survey', 'POST', body);

// 학교 규칙 수립을 위한 조사서 (Tier 1) — 반·학기 단위
export const fetchSchoolRules = (classId, semester = 1) =>
  api(`/api/school-rules-survey?class_id=${encodeURIComponent(classId)}&semester=${encodeURIComponent(semester)}`);
export const saveSchoolRules = (body) => api('/api/school-rules-survey', 'POST', body);

// 학급관리 체크리스트 (Tier 1) — 반·학기 단위
export const fetchClassChecklist = (classId, semester = 1) =>
  api(`/api/class-checklist?class_id=${encodeURIComponent(classId)}&semester=${encodeURIComponent(semester)}`);
export const saveClassChecklist = (body) => api('/api/class-checklist', 'POST', body);
...
// IEPBS 모듈1 (출발점 / 학습자 분석)
export const fetchStartpoint = (sid) => api(`/api/students/${sid}/startpoint`);
export const saveStartpoint = (sid, data) => api(`/api/students/${sid}/startpoint`, 'POST', { data });

// IEP (개별화교육계획) goals
export const fetchIEP = (sid) => api(`/api/students/${sid}/iep`);
export const saveIEPGoal = (sid, body) => api(`/api/students/${sid}/iep`, 'POST', body);
export const deleteIEPGoal = (sid, id) => api(`/api/students/${sid}/iep`, 'DELETE', { id });
...
// 0915(mds/31): 교수 프로그램 회기 기록(과제분석 단계 × 회기)
export const fetchSessions = (sid) => api(`/api/students/${sid}/sessions`);
export const createSession = (sid, body) => api(`/api/students/${sid}/sessions`, 'POST', body);
export const updateSession = (sid, body) => api(`/api/students/${sid}/sessions`, 'PUT', body);
export const deleteSession = (sid, id) => api(`/api/students/${sid}/sessions`, 'DELETE', { id });
export const fetchPrograms = (sid) => api(`/api/students/${sid}/programs`);
export const createProgram = (sid, body) => api(`/api/students/${sid}/programs`, 'POST', body);
export const updateProgram = (sid, body) => api(`/api/students/${sid}/programs`, 'PUT', body);
export const deleteProgram = (sid, id) => api(`/api/students/${sid}/programs`, 'DELETE', { id });
