...
export const RECORD_FRAMES = [
  ...
  },
  {
    ...
    what: '실제 상황에서 기회마다 1~5점', metric: 'scale', status: 'ready',
...
/** 회기에 적을 기회 수 — 실제 상황은 그날 점검한 만큼(0~2번)이라 기본 2칸. */
export const SCALE_TRIAL_OPTIONS = [1, 2, 3, 4, 5];
export const SCALE_TRIALS_DEFAULT = 2;
export const SCALE_MAX_TRIALS = 10;
/** 점수 칸 정규화 — 1~5 정수만 남기고 나머지는 빈칸. 저장 전에 서버가 다시 부른다. */
export function normalizeScores(scores, n = null) {
  const list = Array.isArray(scores) ? scores : [];
  const len = Math.max(1, Math.min(SCALE_MAX_TRIALS, n || list.length || SCALE_TRIALS_DEFAULT));
  return Array.from({ length: len }, (_, i) => {
    const v = Number(list[i]);
    return Number.isFinite(v) && v >= BST_BOTTOM && v <= BST_TOP ? Math.round(v) : '';
  });
}
/** 학습기준 문구 — 실제 상황 5점이 몇 회기 연속인지. */
export const scaleMasteryText = (m = BST_MASTERY) =>
  `실제 상황에서 ${m?.score || BST_TOP}점(스스로) ${m?.runs || BST_MASTERY.runs}회기 연속`;
