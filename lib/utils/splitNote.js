...
// 표시용 중복 제거(0720 현장 피드백: 같은 문장이 강점 칩·어려움 칩·추가 요약에
// 세 번 반복돼 보임). '추가 요약'을 문장 단위로 나눠, 이미 강점/어려움에 들어 있는
// 문장은 걸러낸다. 데이터는 건드리지 않고 화면 표시만 정리한다.
export function dedupeExtra(extra, strengths, difficulties) {
  if (!extra || !String(extra).trim()) return '';
  const norm = (s) => String(s || '').replace(/[\s"'“”‘’.,·;:()[\]!?~\-]/g, '');
  const pool = norm(strengths) + '|' + norm(difficulties);
  const sentences = String(extra)
    .split(/\n+|(?<=[.!?])\s+/)
    .map((s) => s.trim())
    .filter(Boolean);
  const kept = sentences.filter((p) => {
    const n = norm(p);
    if (!n) return false;
    // 아주 짧은 구(학년 등)는 오탐 방지를 위해 유지, 그 외엔 강점/어려움에 포함되면 제외.
    return n.length < 6 ? true : !pool.includes(n);
  });
  return kept.join(' ');
}
