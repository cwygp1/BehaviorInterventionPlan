...
//   bullet=false 이면 현행수준 같은 줄글용 — "- "를 떼지도 붙이지도 않고 줄만 나눈다(문서 출력이 달라지지 않게).

const BULLET = /^\s*[-•·]\s*/;
// 구분은 짧은 이름(공백 포함 8자 이하, "→"·":" 없음)일 때만 뗀다 — 문장 속 콜론을 구분으로 오인하지 않게.
const LABEL = /^([^:：→\n]{1,8}?)\s*[:：]\s*(.*)$/;

export function parseRows(value, labeled = false, bullet = true) {
  const lines = String(value || '').split('\n').map((s) => (bullet ? s.replace(BULLET, '') : s).trim()).filter(Boolean);
...
export function serializeRows(rows, bullet = true) {
  return rows
    .map((r) => {
