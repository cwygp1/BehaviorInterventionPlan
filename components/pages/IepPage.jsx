...
export default function IepPage() {
  ...
  async function buildGenPrompt() {
    ...
    return (
      ...
      `   ① 지도전략 — 이 구간의 교육내용·학생 특성에 맞는 전략을 직접 골라 쓸 것. 후보는 다양하다(개별시도교수(DTT), 자연적 상황 교수(NET), 모델링·비디오 모델링, 시각적 지원, 구조화 교수, 사회적 이야기, 또래 중재, 과제분석 등). 어떤 활동에서 어떤 지시어·상황으로 쓰는지 이 학생의 실제 활동에 맞춰 서술.\n` +
      `   ② 지원수준(촉구·용암) — 촉구 체계와 시기별 용암 계획을 이 학생의 현행수준에 맞게 서술(무엇을 촉구하고 언제 어떻게 줄이는지).\n` +
      `   ③ 강화 스케줄 — 습득 단계와 유지 단계의 강화 계획을 서술하되, 학생 자료에 나온 이 학생이 실제 좋아하는 것(간식·활동·칭찬 등)을 강화물로 연결.\n` +
      `   ※ 위 괄호 예시는 참고일 뿐이다. 예시 문구를 그대로 옮겨 쓰지 말고 매 구간 이 학생·이 내용에 맞게 새로 작성할 것. 단, 3구조의 틀 자체는 구간마다 일관되게 유지하고 세부(활동·촉구 강도·강화 간격)만 점증 조정한다.\n` +
      ...
      `7) [평가계획(eval_plan)] 구간마다 교육목표·교육내용에 근거한 "~는가?" 질문형 항목 2~4개. 반드시 서로 다른 측면을 다각적으로 다룰 것 — (a) 수행·도달(정확히 하는가), (b) 참여 태도(거부감 없이·관심을 보이며 하는가), (c) 지속성(일정 시간·횟수 이상 하는가), (d) 독립·모방 수준(스스로/모방하여 하는가), (e) 일반화(다른 상황·자료·사람에게도 하는가) 중 2~4개 측면을 골라 한 측면당 1개 질문. 같은 측면 반복 금지.\n` +
      `   예: "- 여러 가지 재료를 거부감 없이 만지는가?", "- 다양한 놀잇감을 사용하여 10분 이상 놀이하는가?", "- 요구가 필요한 상황에서 교사를 모방하여 손가락으로 가리키거나 양손을 뻗어 요구를 표현하는가?"\n` +
