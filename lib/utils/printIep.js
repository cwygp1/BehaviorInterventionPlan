...
function critLine(goal) {
  ...
  return `<p class="crit">평가 기준: ${goal.crit_type === 'rate' ? '독립 수행 비율' : `기회 중 성공 횟수(${esc(goal.crit_of || 10)}회 중)`} ${esc(goal.crit_start)}${u} → ${esc(goal.crit_end)}${u} (양적) · 평가초점 중심 질적 서술 병행</p>` + tierLine(goal);
}
...
export function downloadIepWord({ student = {}, teacherName = '', school = '', goals = [] }) {
  ...
  const body = goals
    .map((g, i) => {
      // 0915: 성취기준 없이 저장한 목표는 과목·학년군·성취기준 줄을 비운다(빈 [ ] 표기 방지).
      const stds = [{ code: g.standard_code, text: g.standard_text }, ...(Array.isArray(g.related_stds) ? g.related_stds : [])].filter((x) => x && x.code);
      const title = [g.subject ? esc(g.subject) + (g.area ? ' · ' + esc(g.area) : '') : '', g.subject ? esc(GRADE[g.grade_code] || '') : '', `${g.semester}학기`].filter(Boolean).join(' · ');
      return (
        `<h3>${i + 1}. ${title}</h3>` +
        (stds.length ? `<p class="std">성취기준 ${stds.map((x) => `[${esc(x.code)}] ${esc(x.text || '')}`).join(' / ')}</p>` : '') +
        fociBlock(g) +
  ...
  const css =
    ...
    // 0720: table-layout:fixed — 워드 버전에 따라 열 폭이 널뛰거나 붕괴하는 것 방지(P14 계열).
    'table{border-collapse:collapse;width:100%;margin:6px 0 14px;table-layout:fixed}' +
    'td,th{border:1px solid #000;padding:5px 6px;font-size:10.5pt;vertical-align:top;word-wrap:break-word}' +
...
const critMethodText = (t, of) => (t === 'qual' ? '관찰법(질적·서술 평가)' : t === 'task' ? '과제분석(단계별 수행 평가)' : t === 'freq' ? `관찰법(${of || 10}회 기회 중 성공 횟수)` : '관찰법(독립 수행 비율 %)');

// 0819: 월별 교육방법이 구간마다 달라지므로(점증 사다리), 학기 요약 셀에 전부 나열하면
// 최대 18줄이 된다 → 3구조(지도전략/지원수준/강화)별로 묶어, 여러 구간에 걸쳐 달라진
// 항목은 "(학기초) 첫 구간 ~ (학기말) 마지막 구간" 두 줄로 압축한다.
...
// 0903(B안): 종합표 셀용 — 학기목표 + 성취기준별 목표(설정에 따라).
function goalWithStdGoals(g) {
  const sg = stdGoalsForDoc(g);
  if (!sg.length) return g.semester_goal || '';
  return `${g.semester_goal || ''}\n${sg.map((x) => `- [${x.code}] ${x.goal}`).join('\n')}`;
}
...
function iepFormTable(g) {
  ...
  // 0903(B안): 성취기준별 목표(학생 수준으로 조정한 목표) — 설정이 '한 문장만'이면 생략.
  const sgDoc = stdGoalsForDoc(g);
  if (sgDoc.length) {
    goalParts.push('');
    goalParts.push('＊성취기준별 목표');
    sgDoc.forEach((x) => goalParts.push('- [' + esc(x.code) + '] ' + escML(x.goal)));
  }
  ...
  goalParts.push('＊평가방법: ' + critMethodText(g.crit_type, g.crit_of));
  if (foci.length) {
    goalParts.push('＊평가초점');
  ...
  // 학기 수준 교육방법(P15, 교사 방향)이 있으면 그것이 이 셀의 정답 — 없으면 월별을 압축 요약.
  const methods = (g.sem_methods && String(g.sem_methods).trim())
    ? bulletML(dedupeLines([g.sem_methods]))
    : bulletML(summarizeMethods(g.monthly));
...
export function downloadIepFormWord({ student = {}, teacherName = '', school = '', goals = [] }) {
  ...
  const css =
    LANDSCAPE_CSS +
    'body{font-family:"맑은 고딕",sans-serif}' +
    'table{border-collapse:collapse;width:100%;margin:6px 0 16px;table-layout:fixed}' +
...
export function downloadTaskSheet({ student = {}, teacherName = '', school = '', goalText = '', steps = [], chainType = 'forward', promptSystem = 'mtl', totalCols = 8 }) {
  ...
  const css =
    LANDSCAPE_CSS +
    'body{font-family:"맑은 고딕",sans-serif}' +
    'table{border-collapse:collapse;width:100%;margin:6px 0 12px;table-layout:fixed}' +
...
const REPORT_CSS =
  LANDSCAPE_CSS +
  'body{font-family:"맑은 고딕",sans-serif}' +
  'table{border-collapse:collapse;width:100%;margin:6px 0 18px;table-layout:fixed}' +
...
export function downloadIepReport({ student = {}, teacherName = '', school = '', year, semester = '', goals = [] }) {
  ...
  const semRows = list.map((g) =>
    ...
    `<td>${escML(g.plop)}</td><td>${escML(goalWithStdGoals(g))}</td>` +
