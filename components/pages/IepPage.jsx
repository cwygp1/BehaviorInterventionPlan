...
export default function IepPage({ onNavigate }) {
  ...
  // 0921(현장 요청 "이것도 예제로 활용"): 2025 일상생활 활동 수업 도움 자료의 설계 카드 32장 — 같은 단원의 예시를
  //   지도서 활동 카드 안에 보여 주고, 학기 교육내용·교육방법과 월별 프롬프트에 서술 방식 본보기로 넣는다.
  const [dlExamples, setDlExamples] = useState(null);
  useEffect(() => {
    if (dlExamples || !dlUnits.length) return;
    let alive = true;
    loadDailyLifeExamples().then((d) => { if (alive && d) setDlExamples(d); });
    return () => { alive = false; };
  }, [dlUnits.length, dlExamples]);
  const dlExByUnit = useMemo(() => examplesForUnits(dlExamples, dlUnits), [dlExamples, dlUnits]);
  const dlExList = useMemo(() => interleaveExamples(dlExByUnit, dlUnits), [dlExByUnit, dlUnits]);
  // 0922(1단계): 교과 단원·활동 키워드(기본 국어 1~2학년군 10개) — 일상생활 지도서와 같은 자리·같은 방식.
  //   성취기준을 하나라도 고르면 21KB 파일을 한 번 받고, 데이터가 있는 코드만 항목이 된다(없으면 카드·블록 없음).
  const [ccData, setCcData] = useState(null);
  useEffect(() => {
    if (ccData || !selCodes.length) return;
    let alive = true;
    loadCurriculumContent().then((d) => { if (alive && d) setCcData(d); });
    return () => { alive = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  async function aiSemContentMethods() {
    ...
    try {
      ...
      const prompt =
        ...
        '\n작성 규칙:\n' +
        '1) content(교육내용): 학기목표에 도달하기 위한 구체 활동을 "~하기" 명사형으로 4~7개(위에 [성취기준별 목표]가 있으면 목표마다 1~2개씩). 활동을 잘게 쪼개고 실제 자료·상황을 담을 것. 아래 [지도서 활동]이나 [교과 단원·활동 키워드]가 있으면 그 활동 이름을 소재로 우선 쓸 것(학생 수준에 맞게 고쳐 쓰되 거기 없는 활동을 지어내지 말 것). 학기목표에 여러 요소(예: 읽기와 대화)가 있으면 모든 요소를 고르게 다룰 것. "~하기"는 항목 끝에 1번만 쓸 것("돌리기하기"처럼 겹치면 안 됨).\n' +
  ...
  async function aiGoalFromFunc() {
    ...
    try {
      const bip = curStuData?.bip || {};
      // 0917(햇살 제보): 기능·대체행동·강점을 그대로 이어붙인 나열식 문장(선행조건 → 대체행동 → 후속 강화까지
      //   한 문장, 괄호 설명 그대로)이 나왔다. 원인은 규칙 1·2가 "상황·기준·강점을 모두 문장에 넣으라"고
      //   요구한 데다 유일한 예시 자체가 절 3개짜리 긴 문장이어서 그 뼈대를 그대로 따라 한 것.
      //   → 입력은 재료로만, 조건+행동+기준 한 문장·60~80자 상한, 좋은/나쁜 예 few-shot, 길이·괄호 가드 1회 재시도.
      const basePrompt = (strict) =>
        '너는 특수교육 IEP 작성 전문가다. 아래 대체행동(단기목표)이 자리 잡은 뒤 한 학기 끝에 학생이 보일 "바람직한 행동"을 학기목표(주 목표) 한 문장으로 써라.\n' +
        '규칙:\n' +
        '1) 구조는 "조건(한 가지 상황) + 행동(한 가지) + 기준(하나)"뿐이다. 대체행동보다 한 단계 넓고 자연스러운 행동을 고른다.\n' +
        '2) [가장 중요] 아래 입력값은 재료일 뿐, 문장에 그대로 옮기지 않는다. 문제행동 정의·강점·현행수준의 구절을 베끼지 말고, 괄호 설명은 절대 넣지 않는다. 강화물(좋아하는 활동)을 문장에 넣지 않는다 — 강화는 교육방법 칸에서 다룬다.\n' +
        '3) 한 문장, 60~80자. "~할 때, ~한 뒤, ~하고, 이후 ~" 식으로 절을 세 개 이상 잇지 않는다. 무엇을 측정하는지 한눈에 읽혀야 한다.\n' +
        '4) 강점은 "방법"으로만 녹인다(예: 그림카드로, 몸짓으로). 강점 문장을 옮겨 적지 않는다.\n' +
        '5) "~할 수 있다."로 끝맺음. 부정 진술 금지. 교수전략 이름·영어 단어·"온순하게" 같은 평가어 금지. 쉬운 우리말.\n' +
        '[좋은 예]\n' +
        '· 대체행동 "과제가 어려울 때 휴식 카드를 건넨다" → "어려운 과제에서 휴식 카드로 요청한 뒤 과제를 끝까지 마치는 날이 주 4일 이상일 수 있다." (58자)\n' +
        '· 대체행동 "친구와 놀고 싶을 때 어깨를 두드린다" → "쉬는 시간에 어깨 두드리기로 놀이를 청해 친구와 5분 이상 함께 놀 수 있다." (44자)\n' +
        '[나쁜 예 — 이렇게 쓰지 말 것]\n' +
        '· "과제 수행이나 놀이 정리 요구를 받을 때, 원하는 활동을 그림카드나 몸짓으로 온순하게 요청한 뒤 교사의 안내를 받아 짧은 과제나 정리를 끝까지 수행하고, 이후 좋아하는 시각 활동(위에서 아래로 떨어뜨리고 소리나게 하는 활동)을 하는 날이 주 5일 중 4일 이상일 수 있다." — 입력값 나열, 절 4개, 괄호 설명, 강화물 포함, 130자 초과.\n' +
        (strict ? '※ 직전 답변이 너무 길거나 괄호·입력 구절이 그대로 들어갔다. 이번에는 80자 안에서 조건+행동+기준만 남겨 다시 써라.\n' : '') +
        `[행동 기능] ${funcPlan.func}\n` +
        (bip.opdef ? `[문제행동 정의 — 참고만] ${bip.opdef}\n` : '') +
        `[대체행동(단기목표)] ${alt}\n` +
        `[학생 강점 — 방법으로만 반영] ${strengthsText() || '(미입력)'}\n` +
        (String(plop || '').trim() ? `[현행수준 — 참고만] ${String(plop).trim()}\n` : '') +
        '반드시 JSON만 출력: {"semester_goal":"..."}';
      // 나열식 판정: 100자 초과 또는 괄호 포함 → 1회 재시도(엄격), 그래도 길면 적용하되 경고.
      const tooListy = (t) => t.replace(/\s/g, '').length > 100 || /[()（）]/.test(t);
      let j = await llmJSON('학기목표 생성(기능중심)', basePrompt(false), { tier: 'quality', temperature: 0.3, thinking: true });
      let g = String(j.semester_goal || '').trim();
      if (g && tooListy(g)) {
        try {
          j = await llmJSON('학기목표 생성(기능중심·재시도)', basePrompt(true), { tier: 'quality', temperature: 0.2, thinking: true });
          const g2 = String(j.semester_goal || '').trim();
          if (g2 && (!tooListy(g2) || g2.length < g.length)) g = g2;
        } catch (_) { /* 1차 결과 유지 */ }
      }
      if (!g) throw new Error('학기목표 문장을 받지 못했어요.');
      setGoal(toCanDo(g));
      if (tooListy(g)) toast('⚠ 문장이 아직 길어요 — 조건·행동·기준만 남기고 "AI 문장 다듬기"나 직접 손봐 주세요.');
    ...
    <>
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 8 }}>
          {[
            // 0908: '교과 중심/생활 지원 중심' 꼬리표 제거 — A에서 일상생활 활동 영역을 고르면 생활 지원도 같은 절차로 되므로 구분이 불필요.
          ...
          <FoldCard id="iep-dl-guide" title="📖 지도서 활동 (일상생활 교사용 지도서)" defaultOpen
            ...
            {dlUnits.map((u) => (
              <div key={u.code} style={{ marginBottom: 10 }}>
                ...
                <DailyLifeExampleCards list={dlExByUnit[u.code]} onAddContent={(t) => { addContentLines([t]); toast('교육내용에 넣었어요: ' + t); }} />
              </div>
            ))}
            {dlExList.length > 0 && (
              <div style={{ fontSize: '.74rem', color: 'var(--muted)', marginBottom: 2 }}>예시 출처: 2025 일상생활 활동 수업 도움 자료 활용 안내(설계 카드 32건 중 이 단원 것). 설계 형태 — A 영역 내 선택 · B 영역 간 통합 · C 교과 연계 · D 창의적 체험활동 연계. AI 교육내용·월별 생성에도 서술 방식 본보기로 들어가요.</div>
            )}
                ...
                <div key={e.code} style={{ marginBottom: 10 }}>
                  ...
                  {units.length > 0 && (
                    <div style={{ fontSize: '.8rem', color: 'var(--sub)', margin: '2px 0 6px' }}>
                      단원: {units.map((u, i) => <span key={i} style={{ marginRight: 8 }}>{u.book}{u.unitNo != null ? ` ${u.unitNo}단원` : ''} '{u.unit}'{u.role === 'related' && <span style={{ color: 'var(--muted)' }}>(관련)</span>}</span>)}
                    </div>
                  )}
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 4 }}>
                    {(e.activities || []).map((a, i) => (
                      <button key={i} type="button" className="qchip" title="누르면 교육내용에 추가"
                        onClick={() => { addContentLines([a]); toast('교육내용에 넣었어요: ' + a); }}>
                        {a}
                      </button>
                    ))}
                  </div>
                  {(e.materials || []).length > 0 && <div style={{ fontSize: '.78rem', color: 'var(--sub)', marginTop: 4 }}>자료: {e.materials.join(', ')}</div>}
                </div>
              );
            })}
