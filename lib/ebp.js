...
export const EBP_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      // 0908(2차 피드백 "DTT·직접교수가 왜 고려 대상이 아닌가"): 직접교수(DI)는 27가지 목록에는 없지만
      //   2020년 28가지 목록에 Direct Instruction으로 들어 있고 특수교육 교과 지도의 기본 방법이라 카탈로그에 넣는다.
      //   그동안 lib/disabilityMethods.js의 기본 목록에만 이름이 있어 EBP 후보 추천(selectEBP)에는 한 번도 뽑히지 않았다.
      { n: '직접교수', d: '교사 주도의 명시적 수업 — 설명·시범 → 안내된 연습 → 독립 연습. 읽기·수학 같은 교과(학업) 기술 교수의 기본', area: '교과(읽기·쓰기·수학)·학업', age: '초등~고등학생' },
      { n: '비연속 시행 훈련(DTT)', d: '개별시도교수 — 구조화된 장면에서의 1:1 체계적 교수(ABA 기반). 지시 → 반응 → 즉시 후속결과의 짧은 시행을 여러 번 반복', area: '거의 모든 영역(특히 언어)', age: '영유아~초등 특히 효과', link: 'https://blog.naver.com/bjs718/221440608670' },
      // 0908 추가(27가지 목록 밖, 직접교수와 같은 예외): 대체행동·사회성·안전 같은 "행동으로 보이는 기술"을 가르치는 표준 묶음.
      //   직접교수(교사 주도 명시적 수업)·DTT(1:1 구조화 반복 시행)와 겹쳐 보이지만 별개 방법 — 프롬프트에서 혼용을 막는다.
      { n: '행동기술훈련(BST)', d: '설명(지시) → 시범 → 연습(역할극·리허설) → 피드백을 한 묶음으로 반복해 행동 기술을 가르치는 방법. 사회성·안전·자기관리·대체행동 교수에 사용', area: '사회성·안전·자기관리·일상생활', age: '거의 모든 연령' },
...
export const EBP_CATALOG = [
  ...
  { n: '직접교수', cat: '교수전략', d: '교사 주도 명시적 수업 — 설명·시범 → 안내된 연습 → 독립 연습(교과·학업 기술)', area: '교과(읽기·쓰기·수학)·학업' },
  { n: '비연속 시행 훈련(DTT)', cat: '교수전략', d: '1:1 구조화 장면에서 지시 → 반응 → 즉시 후속결과를 짧은 시행으로 반복(기초 기술 습득 초기)', area: '거의 모든 영역(특히 언어)' },
  { n: '행동기술훈련(BST)', cat: '교수전략', d: '설명→시범→연습(역할극)→피드백 묶음 반복(행동 기술·대체행동 교수)', area: '사회성·안전·자기관리·일상생활' },
...
// 0902: DTT·자연적 중재가 카탈로그에만 있고 어떤 매핑에도 없어 실제로 선택되지 않던 문제 →
// 습득(구조화 1:1)은 DTT, 일반화(일과 속)는 자연적 중재가 후보에 항상 들어가게 한다.
const GOALTYPE_MAP = {
  task: ['과제분석', '촉진', '시간지연', '비디오모델링', '모델링', '강화', '비연속 시행 훈련(DTT)', '자연적 중재(교수)'],
  rate: ['강화', '촉진', '시각적 지원', '시간지연', '행동기술훈련(BST)', '비연속 시행 훈련(DTT)', '자연적 중재(교수)'],
  qual: ['시각적 지원', '사회적 담화(사회적 이야기)', '행동기술훈련(BST)', '자기관리전략', '강화', '자연적 중재(교수)'],
};

// 교육과정 성격 → 우선 추천 교수 방법 (0908 피드백: 교과중심=직접교수 / 일상생활=BST / 기능중심=DTT).
// FUNCTION_MAP·GOALTYPE_MAP과 같은 가중치 방식 — '하드 규칙'이 아니라 후보 순위를 올려 주는 기본값이라
// 교사가 다른 방법을 고르면(장애영역 기본 목록·[학기 교육방법]) 그 방법이 그대로 쓰인다.
const CURRICULUM_MAP = {
  subject: ['직접교수', '과제분석', '시각적 지원', '촉진'],
  daily: ['행동기술훈련(BST)', '과제분석', '자연적 중재(교수)', '촉진'],
  func: ['비연속 시행 훈련(DTT)', '기능적 의사소통 훈련(FCT)', '차별강화', '강화'],
};
 ...
 * @param {string} opts.curriculum    - 'subject'(교과 중심) | 'daily'(일상생활 교육과정) | 'func'(기능 중심)
 * @param {string} opts.text          - 행동/목표/성취기준 등 자유 텍스트(키워드 매칭용)
 * @param {number} opts.max           - 최대 개수(기본 5)
 * @returns {Array} EBP_CATALOG 항목 배열
 */
export function selectEBP({ qabfFunction = '', goalType = '', curriculum = '', text = '', max = 5 } = {}) {
  ...
  // 2-1) 교육과정 성격 매핑(0908) — 첫 항목이 그 교육과정의 기본 교수 방법.
  const cm = CURRICULUM_MAP[curriculum];
  if (cm) cm.forEach((n, i) => bump(n, 4 - i * 0.4));
...
export function ebpBlockForGoal({ goalType = '', goalText = '', qabfFunction = '', curriculum = '', max = 5 } = {}) {
  return buildEbpBlock(selectEBP({ goalType, qabfFunction, curriculum, text: goalText, max }), { title: 'IEP 교육방법 증거기반실제(EBP) 후보' });
}
