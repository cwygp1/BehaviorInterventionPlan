...
function buildRequestBody(model, messages, opts) {
  return {
    ...
    // Reasoning(thinking) 제어. 기본 OFF — 이 앱은 직접 답변(content)을 파싱하고,
    // thinking이 켜지면 사고과정으로 수백~수천 토큰을 먼저 쓴다(3~4배 느림).
    // 0904 실측: QABF 해석·BIP 전체·IEP 학기목표처럼 추론·완성도가 중요한 긴 작업만
    // opts.thinking=true 로 켠다(사고 강도 low). 나머지(채팅·평어·통신문·계산)는 OFF.
    //
    // 서버마다 인식하는 스위치가 달라 세 가지를 함께 보낸다:
    //  - enable_thinking(최상위): mlx-vlm 서버·MTPLX가 인식. (mlx-vlm은 아래 kwargs를 무시)
    //  - reasoning_effort: LM Studio MLX 엔진은 'none' 만이 thinking을 확실히 끈다
    //    (chat_template_kwargs 는 무시함). 켤 때는 'low' — Qwen3.8 기본 xhigh 는 과사고.
    //  - chat_template_kwargs.enable_thinking: llama.cpp(GGUF) 계열 호환용으로 유지.
    ...(opts.thinking === true
      ? { enable_thinking: true, reasoning_effort: opts.reasoning_effort || 'low' }
