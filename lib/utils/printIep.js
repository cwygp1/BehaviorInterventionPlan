...
function critLine(goal) {
  if (goal.crit_type === 'qual') return `<p class="crit">평가 방식: 질적 평가 — 평가초점 중심의 내러티브(서술형) 평가</p>`;
  const u = unitOf(goal.crit_type);
  return `<p class="crit">평가 기준: ${goal.crit_type === 'rate' ? '독립 수행 비율' : '기회 중 성공 횟수'} ${esc(goal.crit_start)}${u} → ${esc(goal.crit_end)}${u} (양적) · 평가초점 중심 질적 서술 병행</p>`;
}
