...
async function runMigrations() {
  ...
  // 사용자 역할(role) — 관리자 페이지·Q&A 게시판 답변 권한용.
  await ensureUserRoleCol();

  // Q&A 게시판 (사람 관리자가 답변) — mds/28 Part B.
  await ensureQaSchema();

  // AI 전문가 채팅 스레드(대화방) — mds/28 Part C.
  await ensureChatThreads();
  ...
  // 학교 규칙 수립을 위한 조사서 (Tier 1) — 반·학기 단위 1행. responses(JSONB).
  // 심각·사소한 문제행동 실태, 활동별 발생 가능성·순위, 필요한 변화 등 5개 문항.
  await rawSql`
    CREATE TABLE IF NOT EXISTS school_rules_survey (
      id SERIAL PRIMARY KEY,
      user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
      class_id INTEGER REFERENCES classes(id) ON DELETE CASCADE,
      semester INTEGER NOT NULL DEFAULT 1,
  ...
  // 학기목표 한 문장은 이 묶음의 요약이고, 평가초점·월별 계획은 이 목록에서 파생한다.
  await rawSql`ALTER TABLE iep_goals ADD COLUMN IF NOT EXISTS std_goals JSONB NOT NULL DEFAULT '[]'`;
  // 0908(기능중심 IEP — 경로B): {func, skill, alt} — 행동 기능·핵심기술·대체행동(단기목표). 경로A 저장본은 {}.
  await rawSql`ALTER TABLE iep_goals ADD COLUMN IF NOT EXISTS func_plan JSONB NOT NULL DEFAULT '{}'`;
  // 0915: '기회 중 성공 횟수' 평가의 분모(기회 수) — 5·10·20. 예전 목표는 10회.
  await rawSql`ALTER TABLE iep_goals ADD COLUMN IF NOT EXISTS crit_of INTEGER NOT NULL DEFAULT 10`;
  ...
  // 0822 워크플로 개편: 행동기능 가설문 + 초기면담지(JSON).
  await rawSql`ALTER TABLE bip_data ADD COLUMN IF NOT EXISTS hypothesis TEXT NOT NULL DEFAULT ''`;
  await rawSql`ALTER TABLE bip_data ADD COLUMN IF NOT EXISTS interview JSONB NOT NULL DEFAULT '{}'`;
  // 0914(홍준표 부록 반영): ABC 배경사건(setting event — 수면 부족·투약 변경 등 그날의 조건) + BIP 실행 역할분담표.
  await rawSql`ALTER TABLE abc_records ADD COLUMN IF NOT EXISTS setting_event VARCHAR(300) DEFAULT ''`;
  await rawSql`ALTER TABLE bip_data ADD COLUMN IF NOT EXISTS roles JSONB NOT NULL DEFAULT '[]'`;
...
// 사용자 역할(role) 컬럼 — 'user'(교사, 기본) | 'admin'(운영자·답변자).
// 관리자 페이지(가입자 관리)와 Q&A 게시판 답변 권한이 사용한다.
// 자가치유 — login/register/me/admin API가 요청 시점에 보정.
//
// 부트스트랩: 관리자 페이지는 관리자만 들어갈 수 있어 최초 1명은 코드가 만들어야
// 한다. 시스템에 관리자가 한 명도 없을 때만 소유자 계정을 자동 승격한다
// (관리자가 이미 있으면 아무것도 하지 않으므로, UI에서 한 승격/해제와 충돌하지 않음).
export const BOOTSTRAP_ADMIN_EMAIL = 'cwygp@naver.com';

// 관리자가 0명일 때만 소유자 계정을 승격한다 — 언제 몇 번을 불러도 안전(멱등).
// login/register가 소유자 이메일에 한해 직접 호출해, 웜 인스턴스에서 캐시된
// ensure가 건너뛰더라도 가입/로그인 즉시 승격이 반영되게 한다.
export async function bootstrapAdminIfNone() {
  await rawSql`
...
export async function ensureUserRoleCol() {
  await rawSql`ALTER TABLE users ADD COLUMN IF NOT EXISTS role VARCHAR(20) NOT NULL DEFAULT 'user'`;
  await bootstrapAdminIfNone();
}

let userRoleColPromise = null;
export function ensureUserRoleColCached() {
  if (!userRoleColPromise) {
    userRoleColPromise = ensureUserRoleCol().catch((err) => {
      userRoleColPromise = null;
      throw err;
    });
  }
  return userRoleColPromise;
}

// ---- Q&A 게시판 (질문 게시판) — mds/28 Part B ------------------------------
// 사람 관리자(role='admin')가 답변하는 게시판. AI 채팅(chatExpert)과는 별개.
//   qa_questions — 질문. user_id는 SET NULL(탈퇴해도 글 유지).
//                  is_private=true면 작성자·관리자만 열람.
//                  status: 'open'(답변대기) | 'answered'(답변완료, 첫 답변 시 전환).
//   qa_answers   — 답변(관리자). 추가 답변/스레드 확장을 위해 별도 테이블.
export async function ensureQaSchema() {
  await rawSql`
  ...
  `;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_qa_questions_status ON qa_questions(status, id)`;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_qa_questions_user ON qa_questions(user_id)`;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_qa_answers_question ON qa_answers(question_id, id)`;
}

let qaSchemaPromise = null;
export function ensureQaSchemaCached() {
  if (!qaSchemaPromise) {
    qaSchemaPromise = ensureQaSchema().catch((err) => {
      qaSchemaPromise = null;
      throw err;
    });
  }
  return qaSchemaPromise;
}

// ---- AI 전문가 채팅 스레드(대화방) — mds/28 Part C --------------------------
// 메시지는 기존 chat_history를 재사용하고 thread_id 컬럼만 추가한다
// (thread_id NULL = 스레드 도입 전 과거 기록).
//   chat_threads.mode — 전문가 모드: 'pbs' | 'iep' | 'crisis' | 'free' | 'guide'(사이트 사용법)
//   chat_threads.student_id — (P3 대비) 학생 맥락 상담용 소프트 FK, nullable.
export async function ensureChatThreads() {
  await rawSql`
    CREATE TABLE IF NOT EXISTS chat_threads (
      id SERIAL PRIMARY KEY,
      user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
      title VARCHAR(200) DEFAULT '',
      mode VARCHAR(30) DEFAULT 'pbs',
      student_id INTEGER,
      created_at TIMESTAMP DEFAULT NOW(),
      updated_at TIMESTAMP DEFAULT NOW()
    )
  `;
  await rawSql`ALTER TABLE chat_history ADD COLUMN IF NOT EXISTS thread_id INTEGER REFERENCES chat_threads(id) ON DELETE CASCADE`;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_chat_history_thread ON chat_history(thread_id, id)`;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_chat_threads_user ON chat_threads(user_id, updated_at)`;
}

let chatThreadsPromise = null;
export function ensureChatThreadsCached() {
  if (!chatThreadsPromise) {
    chatThreadsPromise = ensureChatThreads().catch((err) => {
      chatThreadsPromise = null;
      throw err;
    });
  }
  return chatThreadsPromise;
}
...
export async function ensureStudentProfileCols() {
  ...
  // 0824(온보딩): 샘플 체험용 학생 표시. TRUE인 학생은 '샘플 삭제'로 일괄 정리된다.
  await rawSql`ALTER TABLE students ADD COLUMN IF NOT EXISTS is_sample BOOLEAN DEFAULT FALSE`;
}
...
//
// 시드 단가(USD / 100만 토큰) — 2026-09-03 각 사 공식 가격표 기준(표준 등급, 캐시/배치 할인 미적용).
// PRICING_SEED_VERSION 을 올리면 다음 요청 때 시드 행은 upsert(단가·메모·정렬 갱신)되고,
// RETIRED_MODELS 에 적힌 구형 행은 삭제된다. 관리자가 UI에서 직접 추가한 행은 건드리지 않는다.
// 같은 버전 안에서는 ON CONFLICT DO NOTHING 이라 관리자 수정값이 유지된다.
export const PRICING_SEED_VERSION = '2026-09-03';

const SEED_MODEL_PRICING = [
  // [provider, model, input$/1M, output$/1M, note, sort]
  // Anthropic (Claude)
  ['Anthropic', 'Claude Fable 5.1', 10.0, 50.0, '최상위', 9],
  ['Anthropic', 'Claude Opus 5', 5.0, 25.0, '플래그십', 10],
  ['Anthropic', 'Claude Sonnet 5', 2.0, 10.0, '최신', 11],
  ['Anthropic', 'Claude Haiku 4.5', 1.0, 5.0, '경량', 12],
  // OpenAI (GPT) — GPT-5.6 계열 2026-07 GA, 2026-07-30 Terra/Luna 인하가 반영.
  ['OpenAI', 'GPT-5.6 Sol', 5.0, 30.0, '플래그십(2026-07)', 20],
  ['OpenAI', 'GPT-5.6 Terra', 2.0, 12.0, '', 21],
  ['OpenAI', 'GPT-5.6 Luna', 0.2, 1.2, '경량', 22],
  ['OpenAI', 'GPT-5.4 mini', 0.75, 4.5, '레거시', 23],
  ['OpenAI', 'GPT-5.4 nano', 0.2, 1.25, '최저가 · 레거시', 24],
  // Google (Gemini) — 3.6/3.7/3.8 Flash는 2027-01-01부터 2배 인상 예고.
  ['Google', 'Gemini 3.1 Pro', 2.0, 12.0, '최신 Pro · ≤200k', 30],
  ['Google', 'Gemini 3.8 Flash', 0.75, 3.75, '최신(2026-09) · 2027-01 2배 인상 예정', 31],
  ['Google', 'Gemini 3.7 Flash', 0.75, 3.75, '2027-01 2배 인상 예정', 32],
  ['Google', 'Gemini 3.5 Flash-Lite', 0.3, 2.5, '경량', 33],
  ['Google', 'Gemini 2.5 Flash-Lite', 0.1, 0.4, '최저가', 34],
  // Zhipu (GLM)
  ['Zhipu', 'GLM-5.3', 1.4, 4.4, '최신(2026-08)', 40],
  ['Zhipu', 'GLM-5.3-Flash', 0.08, 0.25, '경량', 41],
  ['Zhipu', 'GLM-5.2', 1.4, 4.4, '', 42],
  ['Zhipu', 'GLM-4.7', 0.6, 2.2, '', 43],
  // Alibaba (Qwen) — International(싱가포르) 엔드포인트 기준. 중국(베이징) 엔드포인트는 더 저렴.
  ['Alibaba', 'Qwen3.8-Max', 2.0, 6.0, '최신(2026-08)', 50],
  ['Alibaba', 'Qwen3.7-Plus', 0.4, 1.6, '≤256k · 한시 할인가', 51],
  ['Alibaba', 'Qwen3.6-Plus', 0.5, 3.0, '', 52],
  ['Alibaba', 'Qwen3.7-Flash', 0.03, 0.13, '≤32k · 최저가', 53],
  // DeepSeek — 피크(UTC 01-04, 06-10 평일 = KST 10-13, 15-19시) 단가. 비피크는 50%.
  ['DeepSeek', 'DeepSeek-V4-Pro', 1.32, 3.96, '피크 기준 · 비피크 50%', 60],
  ['DeepSeek', 'DeepSeek-V4-Flash', 0.44, 1.32, '피크 기준 · 비피크 50%', 61],
];

// 이전 시드에 있었으나 후속 모델로 대체된 행 — 버전 갱신 시 삭제.
...
async function seedModelPricing() {
  const verR = await rawSql`SELECT value FROM app_settings WHERE key = 'pricing_seed_version'`;
  const current = verR.rows[0]?.value || '';
  const refresh = current !== PRICING_SEED_VERSION;

  if (refresh) {
    for (const [provider, model] of RETIRED_MODELS) {
      await rawSql`DELETE FROM ai_model_pricing WHERE provider = ${provider} AND model = ${model}`;
    }
  }
  ...
  for (const [provider, model, inp, outp, note, sort] of SEED_MODEL_PRICING) {
    if (refresh) {
      await rawSql`
        INSERT INTO ai_model_pricing (provider, model, input_per_mtok, output_per_mtok, note, sort_order, active)
        VALUES (${provider}, ${model}, ${inp}, ${outp}, ${note}, ${sort}, true)
  ...
  if (refresh) {
    await rawSql`
      ...
      VALUES ('pricing_seed_version', ${PRICING_SEED_VERSION}, NOW())
...
// 0915(mds/31): 교수 프로그램 회기 기록 — 과제분석 단계 × 회기. 코드 + · − · P · I(갑 결정) + T(교사 수행).
export async function ensureProgramSessions() {
  await rawSql`
    CREATE TABLE IF NOT EXISTS program_sessions (
      id SERIAL PRIMARY KEY,
      student_id INTEGER REFERENCES students(id) ON DELETE CASCADE,
      goal_id INTEGER,
      goal_ref VARCHAR(10) NOT NULL DEFAULT 'iep',
      date DATE NOT NULL,
      session_no INTEGER NOT NULL DEFAULT 1,
      phase VARCHAR(12) NOT NULL DEFAULT 'teach',
      chain_type VARCHAR(20) NOT NULL DEFAULT 'forward',
      target_step INTEGER,
      steps_snapshot JSONB NOT NULL DEFAULT '[]',
      codes JSONB NOT NULL DEFAULT '[]',
      correct_count INTEGER DEFAULT 0,
      indep_count INTEGER DEFAULT 0,
      prompted_count INTEGER DEFAULT 0,
      scored_count INTEGER DEFAULT 0,
      pct NUMERIC(5,1) DEFAULT 0,
      note TEXT DEFAULT '',
      created_at TIMESTAMP DEFAULT NOW()
    )
  `;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_program_sessions_goal ON program_sessions(student_id, goal_id, date)`;
  // 0915(mds/32): 기록 형태 확장 — 과제분석(chain) 외 DTT 등. 기존 행은 자동으로 'chain'.
  await rawSql`ALTER TABLE program_sessions ADD COLUMN IF NOT EXISTS kind VARCHAR(12) NOT NULL DEFAULT 'chain'`;
  await rawSql`ALTER TABLE program_sessions ADD COLUMN IF NOT EXISTS program_id INTEGER`;
  await rawSql`ALTER TABLE program_sessions ADD COLUMN IF NOT EXISTS item_stats JSONB NOT NULL DEFAULT '[]'`;
  await rawSql`
    CREATE TABLE IF NOT EXISTS teaching_programs (
      id SERIAL PRIMARY KEY,
      student_id INTEGER REFERENCES students(id) ON DELETE CASCADE,
      goal_id INTEGER,
      kind VARCHAR(12) NOT NULL DEFAULT 'dtt',
      title VARCHAR(200) NOT NULL DEFAULT '',
      antecedent TEXT NOT NULL DEFAULT '',
      items JSONB NOT NULL DEFAULT '[]',
      reinforcement TEXT NOT NULL DEFAULT '',
      trials INTEGER NOT NULL DEFAULT 10,
      prompt_method VARCHAR(20) NOT NULL DEFAULT 'slp',
      error_correction VARCHAR(20) NOT NULL DEFAULT 'redo',
      presentation VARCHAR(12) NOT NULL DEFAULT 'rotate',
      mastery JSONB NOT NULL DEFAULT '{"type":"rate","pct":80,"runs":2}',
      status VARCHAR(12) NOT NULL DEFAULT 'active',
      created_at TIMESTAMP DEFAULT NOW(),
      updated_at TIMESTAMP DEFAULT NOW()
    )
  `;
  await rawSql`CREATE INDEX IF NOT EXISTS idx_teaching_programs_student ON teaching_programs(student_id)`;
}

let programSessionsPromise = null;
export function ensureProgramSessionsCached() {
  if (!programSessionsPromise) {
    programSessionsPromise = ensureProgramSessions().catch((err) => {
      programSessionsPromise = null;
      throw err;
    });
  }
  return programSessionsPromise;
}
