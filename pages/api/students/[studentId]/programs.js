...
export default requireStudentAccess(async function handler(req, res) {
  ...
  function clean(b) {
    ...
    const items = (Array.isArray(b.items) ? b.items : [])
      ...
      .slice(0, 20);
    const trials = TRIAL_OPTIONS.includes(Number(b.trials)) ? Number(b.trials) : 10;
    const m = b.mastery || {};
    const runs = Math.max(1, Math.min(5, Number(m.runs) || 2));
    const mastery = m.type === 'count'
  ...
  try {
    ...
    switch (req.method) {
      ...
      case 'PUT': {
        const b = req.body || {};
        const v = clean(b);
        if (!v.title) return res.status(400).json({ error: '프로그램 이름을 적어 주세요.' });
        if (!v.items.length) return res.status(400).json({ error: '표적(학습 항목)을 하나 이상 적어 주세요.' });
        if (v.goal_id) {
        ...
        const r = await sql`
          ...
          VALUES (${studentId}, ${v.goal_id}, 'dtt', ${v.title}, ${v.antecedent}, ${JSON.stringify(v.items)}::jsonb, ${v.reinforcement}, ${v.trials},
      ...
      case 'DELETE': {
        ...
        await sql`DELETE FROM program_sessions WHERE program_id = ${id} AND student_id = ${studentId} AND kind = 'dtt'`;
