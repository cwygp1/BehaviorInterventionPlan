...
// 0915: IEP 목표에서 DTT 프로그램 기본값 — 기회 중 성공 횟수(freq)면 그 분모를 기회 수로, 학기말 목표 횟수를 학습기준으로.
// 독립 수행 비율(rate)이면 학기말 목표 %를 학습기준으로. 예전 freq 목표가 %로 저장돼 있으면(분모보다 큼) 비율로 환산.
export function dttDefaultsFromGoal(g, trials = 10) {
  const end = Number(g?.crit_end);
  if (g?.crit_type === 'freq') {
    const of = TRIAL_OPTIONS.includes(Number(g.crit_of)) ? Number(g.crit_of) : (TRIAL_OPTIONS.includes(Number(trials)) ? Number(trials) : 10);
    const raw = end > of ? Math.round((end / 100) * of) : Math.round(end);
    const min = end > 0 ? Math.max(1, Math.min(of, raw)) : Math.round(of * 0.8);
    return { trials: of, mastery: { type: 'count', of, min, runs: 2 } };
  }
  const t = TRIAL_OPTIONS.includes(Number(trials)) ? Number(trials) : 10;
  const pct = g?.crit_type === 'rate' && end > 0 ? Math.max(1, Math.min(100, Math.round(end))) : 80;
  return { trials: t, mastery: { type: 'rate', pct, runs: 2 } };
}
...
// 0915 보완: 표적 하나의 판정을 한곳에서 — 화면 알림은 셋 중 하나라도 참이면 보인다.
//   reached: 최근 runs회기(기초선 제외) 모두 기준 이상 · stuck: 지도 3회기 연속 50% 미만
//   dropped: 유지·일반화 표적이 기준 아래로 내려감 → '다시 지도' 제안
//   (이전에는 알림 줄이 reached·stuck일 때만 그려져, 유지 표적이 60~70%로 떨어져도 '다시 지도'가 뜨지 않았다.)
export function dttItemJudgement(status, hist, mastery) {
  const h = hist || [];
  const reached = h.length > 0 && masteryReached(h, mastery);
  const stuck = h.length > 0 && stuckItem(h);
  const dropped = status === 'maintain' && h.some((x) => x.phase !== 'baseline') && !reached;
  return { reached, stuck, dropped };
}
...
export function dttSummary(items, sessions, mastery) {
  ...
  if (mastery?.type === 'count') {
    // 0915 보완: 횟수형은 표적별로 — IEP '기회 중 성공 횟수' 목표는 기술(표적) 하나의 "10회 중 8회" 단위라,
    // 표적을 합친 "30회 기회 중 21회"는 목표 기준과 맞지 않는다.
    const snap = Array.isArray(last.steps_snapshot) ? last.steps_snapshot : [];
    const st = Array.isArray(last.item_stats) ? last.item_stats : [];
    const per = snap.map((x, i) => (st[i] && st[i].scored ? `'${x.text}' ${countText(st[i].correct, st[i].scored)}` : '')).filter(Boolean);
    return `지도 ${teach.length}회기 · 최근 회기 ${per.length ? per.join(', ') : countText(last.correct_count, last.scored_count)} 성공${got}`;
  }
