...
export function printBIP({ studentId, level, disability, note, teacherName, school, bip }) {
  ...
  // 0914(홍준표 부록): 실행 역할분담표 — 과제·담당·기간·점검일·결정 구조. 비어 있으면 생략.
  const esc = (v) => String(v ?? '').replace(/[&<>]/g, (ch) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[ch]));
  const rolesTable = (roles) => {
    const rows = (Array.isArray(roles) ? roles : []).filter((r) => r && Object.values(r).some((v) => String(v || '').trim()));
    if (!rows.length) return '';
    return `
    <h2 style="font-size:1.05rem;color:#3a54cf;margin:6px 0 10px;letter-spacing:1px">🤝 실행 역할분담</h2>
    <table style="width:100%;border-collapse:collapse;font-size:.86rem;margin-bottom:18px;page-break-inside:avoid">
      <thead><tr style="background:#f0f3ff">${['과제(무엇을)', '담당(누가)', '기간', '점검일', '결정(유지·수정·종료 기준)'].map((h) => `<th style="border:1px solid #d9e0ee;padding:6px 8px;text-align:left;font-weight:700;color:#3a54cf">${h}</th>`).join('')}</tr></thead>
      <tbody>${rows.map((r) => `<tr>${['task', 'owner', 'period', 'check', 'decision'].map((k) => `<td style="border:1px solid #e4e9f2;padding:6px 8px;vertical-align:top">${esc(r[k]) || '<span style=\"color:#bbb\">—</span>'}</td>`).join('')}</tr>`).join('')}</tbody>
    </table>`;
  };
...
<h2 style="font-size:1.05rem;color:#3a54cf;margin-bottom:12px;letter-spacing:1px">📜 중재 전략 — PTR (Prevent · Teach · Reinforce) + 반응 절차</h2>
${section('🛡 예방 전략 (Prevent)', b.prev, '🛡', '#0a7d4e')}
${section('📖 교수 전략 (Teach)', b.teach, '📖', '#4f6bed')}
${section('⭐ 강화 전략 (Reinforce)', b.reinf, '⭐', '#f59f00')}
