...
export const KINDS = {
  abc: {
    label: 'ABC 관찰', color: T3, soft: SECTIONS.t3.soft, page: 'observe',
    tag: (e) => `ABC ${e.n}`,
    detail: (e) => `${e.n}건${e.info.beh ? ' · ' + e.info.beh : ''}`,
  },
  mon: {
    label: '행동 데이터', color: T3, soft: SECTIONS.t3.soft, page: 'monitor', monitorTab: 'behavior',
    tag: (e) => `행동 ${e.info.freq ?? 0}회`,
    detail: (e) => `${e.info.beh || '표적행동'} ${e.info.freq ?? 0}회${e.info.alt ? ` · 대체행동 ${e.info.alt}회` : ''}`,
  },
  sz: {
    label: '진정 공간', color: T3, soft: SECTIONS.t3.soft, page: 'crisis', warn: true,
    tag: () => '진정 공간',
    detail: (e) => [
      e.n > 1 ? `${e.n}회` : '',
  ...
  fid: {
    label: '실행 충실도', color: T3, soft: SECTIONS.t3.soft, page: 'monitor', monitorTab: 'behavior',
    tag: (e) => `충실도 ${e.info.score ?? 0}/${e.info.total ?? 4}`,
    detail: (e) => `중재 실행 점검 ${e.info.score ?? 0}/${e.info.total ?? 4}`,
  },
  cico: {
    label: 'CICO', color: T2, soft: SECTIONS.t2.soft, page: 'tier2',
  ...
  session: {
    label: '회기 기록', color: IEP, soft: SECTIONS.iep.soft, page: 'monitor', monitorTab: 'sessions',
...
export const MISSING = {
  cico: { text: 'CICO를 하는 중인데 이날 점검표가 없어요.', page: 'tier2' },
  mon: { text: '관찰 기간 중인데 이날 행동 데이터가 없어요.', page: 'monitor', monitorTab: 'behavior' },
};
