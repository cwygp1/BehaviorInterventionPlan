...
### 9.4 워크플로 / Phase별 진행 순서

전체를 Phase 0 → 5로 나누어 단계적으로 구현. 각 Phase는 다음 Phase의 전제 조건이 됨.

**Phase 0 — LLM 백엔드 연결 인프라** (필수 선행)

A1~A6 같은 AI 보조 기능이 실제로 동작하려면 LLM 호출 통로가 필요하다. 사용자 PC의 LM Studio(또는 OpenAI 호환 API 서버)를 **브라우저에서 직접** 호출하는 구조로 설계 — 이유는 Vercel serverless가 사용자 localhost를 못 부르므로.

| 작업 | 내용 |
|------|------|
| 토픽바 LLM 인디케이터 | 🟢 연결됨 / 🟡 미설정 / 🔴 끊김. 클릭 시 설정 모달 |
| AI 설정 모달 | endpoint URL · 모델명 · 시스템 프롬프트(선택) · "연결 테스트" 버튼 |
| `localStorage` 저장 | 키 `seai.llm.config = {endpoint, model}` |
| 헬퍼 함수 | `callLLM(prompt, opts)` — fetch 직접 호출, 타임아웃, 에러 처리 |
| CORS 안내 | LM Studio의 "Cross-Origin Resource Sharing" 옵션 ON 안내 |
