...
export default requireAuth(async function handler(req, res) {
  // 자가치유: /api/migrate 전에도 used_tiers 컬럼 조회/수정이 동작하도록 보정.
  await ensureUserTierColCached();

  if (req.method === 'GET') {
    const result = await sql`
      SELECT id, email, name, school, used_tiers FROM users WHERE id = ${req.userId}
    `;
  ...
  if (req.method === 'PATCH') {
    ...
    const result = await sql`
      ...
      RETURNING id, email, name, school, used_tiers
    `;
    if (result.rows.length === 0) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
